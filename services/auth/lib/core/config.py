...
class AuthModuleConfig(BaseConfig):
    ...
    default_user_role: str = Field(default="user", description="Default role for new users")

    # Session settings
    session_timeout_minutes: int = Field(default=60, description="Session timeout")
    allow_multiple_sessions: bool = Field(default=True, description="Allow multiple active sessions")
...
class AuthConfig(BaseConfig):
    ...
    oauth_providers: list[str] = Field(default_factory=list, description="OAuth provider list")
