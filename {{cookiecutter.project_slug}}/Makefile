...
.PHONY: help quickstart new-project template-help init-template seed-labels template-setup docs-serve docs-build docs-publish \
	boot fmt lint test test-unit test-integration test-service test-service-unit test-service-integration \
...
test-unit: ## Run unit tests for all services
	...
	./pants test services/*/tests/unit::

test-integration: ## Run integration tests (requires infrastructure)
	@echo "🔗 Running integration tests..."
	@echo "⚠️  Make sure infrastructure is running: make dev-up && supabase start"
	ENV=test LOCALSTACK=true ./pants test services/*/tests/integration::

test-service: ## Test specific service (S=service_name)
	@if [ -z "$(S)" ]; then \
		echo "Error: Service name required. Usage: make test-service S=auth"; \
		exit 1; \
	fi
	@echo "🧪 Testing service: $(S)"
	./pants test services/$(S)/tests::

test-service-unit: ## Test specific service unit tests (S=service_name)
	@if [ -z "$(S)" ]; then \
		echo "Error: Service name required. Usage: make test-service-unit S=auth"; \
		exit 1; \
	fi
	@echo "🔬 Testing $(S) unit tests..."
	./pants test services/$(S)/tests/unit::

test-service-integration: ## Test specific service integration tests (S=service_name)
	@if [ -z "$(S)" ]; then \
		echo "Error: Service name required. Usage: make test-service-integration S=auth"; \
		exit 1; \
	fi
	@echo "🔗 Testing $(S) integration tests..."
	ENV=test LOCALSTACK=true ./pants test services/$(S)/tests/integration::

test-coverage: ## Generate coverage report for all services
	@echo "📊 Generating coverage report..."
	./pants test services/:: --test-use-coverage
	coverage report

test-coverage-service: ## Generate coverage report for specific service (S=service_name)
	@if [ -z "$(S)" ]; then \
		echo "Error: Service name required. Usage: make test-coverage-service S=auth"; \
		exit 1; \
	fi
	@echo "📊 Generating coverage for service: $(S)"
	./pants test services/$(S)/tests:: --test-use-coverage
	coverage report --include="services/$(S)/*"
...
test-watch: ## Run tests in watch mode (S=service_name, defaults to all)
	@if [ -n "$(S)" ]; then \
		echo "👀 Watching tests for service: $(S)"; \
		./pants test services/$(S)/tests:: --loop; \
	else \
		echo "👀 Watching all service tests..."; \
		./pants test services/:: --loop; \
	fi
...
new-module: ## Create a new module within a service (e.g., make new-module S=auth M=permissions)
	@if [ -z "$(S)" ] || [ -z "$(M)" ]; then \
		echo "Error: Service and module names required. Usage: make new-module S=auth M=permissions"; \
		exit 1; \
	fi
